...
def generate_report(symbol: str, processed_events: List[Dict[str, Any]], output_file: str):
    ...
    with open(output_file, 'w') as f:
        ...
        # Table header
        f.write("-" * 110 + "\n")
        f.write(f"{'Date/Time':<20} {'Side':<6} {'Qty':<12} {'Price':<12} {'Cost Basis':<15} {'Status':<10} {'Type':<6} {'Position':<12} {'Avg Cost':<12} {'Profit':<15}\n")
        f.write("-" * 110 + "\n")
        ...
        for pe in processed_events:
            ...
            # Write row
            f.write(f"{date_time:<20} {side:<6} {qty_str:<12} {price_str:<12} {cost_basis_str:<15} "
                   f"{status_icon} {status:<9} {position_type:<6} {position_str:<12} {avg_cost_str:<12} {profit_str:<15}\n")
